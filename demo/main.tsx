...
function App() {
  ...
  const pageTitle = icon ? labelFor(icon.name) : doc ? docLabel(doc) : route.path === '/ai' ? 'For AI agents' : route.path === '/motion' ? 'Motion studio' : route.path === '/' ? 'Animated SVG icons' : 'Page not found';
  useEffect(() => {
  ...
  return <div id="top" className="app" data-motion={motion} style={{ '--accent': color, '--icon-color': color, '--enter': `${TIMING.enter}ms`, '--exit': `${TIMING.dismiss}ms` } as CSSProperties}>
    ...
    <header className="site-header multi-page-header"><Link className="brand" href="/" aria-label="Dither Icons home"><span className="brand-mark"><DitherIcon name="layers" size={25} animate={false} /></span>dither<span className="brand-muted">icons</span></Link><nav aria-label="Main navigation">{links.map(link => <Link key={link.href} href={link.href} aria-current={link.active ? 'page' : undefined}>{link.label}{link.label === 'Icons' && <span className="nav-count">{definitions.length}</span>}</Link>)}</nav>
      <div className="header-actions"><button className="header-search" onClick={() => setSearch(true)} aria-label="Search the library"><Glyph name="search" size={15} /><span>Search</span><kbd>⌘ K</kbd></button><button className="icon-button theme-toggle desktop-theme" onClick={() => setDark(value => !value)} aria-label={`Switch to ${dark ? 'light' : 'dark'} theme`}><span key={String(dark)}><Glyph name={dark ? 'sun' : 'moon'} /></span></button><Link className="button subtle header-download" href="/docs/installation">Install package<Glyph name="down" size={15} /></Link><GitHubStars /><SocialLinks /><details ref={mobileMenu} className="mobile-menu"><summary aria-label="Open navigation"><Glyph name="menu" size={21} /></summary><nav aria-label="Mobile navigation">{links.map(link => <Link href={link.href} key={link.href} aria-current={link.active ? 'page' : undefined}>{link.label}<Glyph name="arrow" size={14} /></Link>)}<Link href="/docs/installation">Install package<Glyph name="down" size={14} /></Link><button className="mobile-theme" onClick={() => setDark(value => !value)}><span>Switch to {dark ? 'light' : 'dark'} theme</span><Glyph name={dark ? 'sun' : 'moon'} size={17} /></button><SocialLinks mobile /></nav></details></div>
    </header>
    <main id="main-content"><PageBoundary key={route.path}><Suspense fallback={<div className="page-loading" role="status"><span />Loading page…</div>}><LoadedPage key={route.path}>{route.path === '/' ? <HomePage /> : icon ? <IconPage key={icon.name} name={icon.name} /> : doc ? <DocsPage key={doc.slug} slug={doc.slug} /> : route.path === '/ai' ? <AIPage /> : route.path === '/motion' ? <MotionPage /> : <div className="page-error"><DitherIcon name="search" size={64} animate={false} /><h1 data-page-heading tabIndex={-1}>Page not found</h1><p>This address does not match an icon or guide. Browse the icons to continue.</p><Link href="/" className="button primary">Back to the collection<Glyph name="arrow" size={16} /></Link></div>}</LoadedPage></Suspense></PageBoundary></main>
    <footer className="site-footer"><Link className="brand" href="/"><span className="brand-mark"><DitherIcon name="layers" size={23} animate={false} /></span>dither<span className="brand-muted">icons</span></Link><span>Made by <a href="https://github.com/vijayksingh" target="_blank" rel="noopener noreferrer">Vijay Singh</a>.</span><nav aria-label="Footer navigation"><Link href="/docs">Documentation</Link><Link href="/ai">For AI agents</Link><a href="/llms.txt">llms.txt</a><a href="https://github.com/vijayksingh/dither-icons" target="_blank" rel="noopener noreferrer">Source code</a></nav></footer>
