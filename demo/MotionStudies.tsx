...
export function MotionStudies({texture,enabled}:{texture:Texture;enabled:boolean}){
 const [set,setSet]=useState('Cognimated reader controls');
 ...
 return <section className="motion-studies" id="motion-studies-panel" aria-labelledby="studies-heading">
  ...
  <div className="study-grid">{ORDER.map((name,index)=><article className="study" key={name}>
   <button disabled={!enabled} className="study-stage di-trigger" aria-label={`Play ${labelFor(name)} animation`} style={{'--study-time':`${studies[name].duration/speed}ms`} as React.CSSProperties}>
  ...
  </article>)}</div><p className="study-hint">Hover, focus, or tap an icon to play. Repeated input is ignored until its animation finishes.</p>
 </section>
}
