...
import { packageDownloadUrl } from '../model';

function LiveExample({ kind }: { kind: 'textures' | 'button' | 'replay' }) {
  const { motion } = useSite();
  const [replay, setReplay] = useState(0);
  return <div className="guide-live-example"><span className="example-label"><i />Live example</span><button className="example-action di-trigger" onClick={() => setReplay(value => value + 1)}><DitherIcon name={kind === 'replay' ? 'sparkles' : 'download'} size={36} replayKey={replay} animate={motion} /><span>{kind === 'replay' ? 'Replay animation' : 'Preview download'}</span></button><small>Hover, focus, or tap to preview.</small></div>;
}
...
export default function DocsPage({ slug }: { slug: string }) {
  ...
  return <DirectoryLayout>
    ...
    <div className="doc-reading-layout"><article className="guide-article">
      {slug === 'installation' && <a href={packageDownloadUrl} download className="button primary guide-download"><Glyph name="down" size={16} />Download React package</a>}
      ...
      <div className="guide-source-link"><a href={`/docs/${slug || 'introduction'}.md`} download><Glyph name="code" size={15} />Download this page as Markdown<Glyph name="down" size={14} /></a></div>
