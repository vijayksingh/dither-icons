...
export default function IconPage({ name }: { name: string }) {
  ...
  return <DirectoryLayout icons>
    ...
    <section className="gesture-section" aria-labelledby="gesture-title">
      <div className="gesture-heading"><div><h2 id="gesture-title">Animation sequence</h2></div><span className="duration-tag">{(study.duration / 1000).toFixed(2)}s</span></div>
      ...
      <div className="gesture-controls"><button className="button subtle" aria-pressed={progress !== undefined} disabled={!motion} onClick={() => { setTab('Preview'); setProgress(value => value === undefined ? 0 : undefined); }}><Glyph name="search" size={14} />Inspect timing</button><label><span className="sr-only">Playback speed</span><select aria-label="Playback speed" value={speed} onChange={event => setSpeed(Number(event.target.value))}><option value={1}>Actual speed</option><option value={0.5}>Half speed</option></select></label><span>{progress === undefined ? 'Select Inspect timing to pause playback.' : 'Drag the slider to inspect a frame.'}</span></div>
