...
import { filterIcons, installCommand, labelFor, packageUrl, packageVersion, palettes, reactCode, textures } from './model';
...
export default function HomePage() {
  ...
  return <>
      <section className="hero" aria-labelledby="hero-title">
        ...
        <h1 id="hero-title" data-page-heading tabIndex={-1}>Dithered icons.<br />{' '}<span>In motion.</span></h1>
        <p>{definitions.length} animated icons in dither, solid, and outline.<br />{' '}Use them in React or download an SVG.</p>
        <div className="hero-actions"><a className="button primary" href="#collection">Browse icons<Glyph name="arrow" size={16} /></a><a className="button subtle" href={packageUrl}><Glyph name="code" size={16} />View on npm</a></div>
      ...
      <section id="collection" className="collection" aria-labelledby="collection-title">
        <div className="collection-heading"><div><h2 id="collection-title">Browse the icons</h2><p>Hover or focus to preview. Select an icon for code and SVG downloads.</p></div><label className="motion-toggle"><input type="checkbox" checked={motion} onChange={event => setMotion(event.target.checked)} /><span className="switch-track"><span /></span><span>Motion {motion ? 'on' : 'off'}</span></label></div>
        ...
        <div className="category-row"><CategoryFilter value={category} onChange={value => { setCategory(value); setShowAll(false); }} motion={motion} /><span className="result-count" role="status">{filtered.length} {filtered.length === 1 ? 'icon' : 'icons'}</span></div>
        ...
        <div className="gallery-foot">{visible.length < filtered.length ? <button className="button subtle show-all" disabled={expanding} onClick={() => startExpansion(() => setShowAll(true))}>{expanding ? 'Loading icons…' : `Show all ${filtered.length} icons`}<span>+{filtered.length - visible.length}</span><Glyph name="arrow" size={16} /></button> : null}</div>
      </section>

      <section className="motion-invitation" aria-label="Motion study invitation"><div className="invitation-art di-trigger" tabIndex={0} role="img" aria-label="Animated layers illustration"><PreviewIcon name="layers" size={64} texture={texture} animate={motion} /></div><div><h2>See how they move.</h2><p>The bell swings. The download arrow lands in its tray.<br />{' '}Replay the animations or inspect them frame by frame.</p></div><Link className="button subtle" href="/motion">Open motion studio<Glyph name="arrow" size={16} /></Link></section>


      <section id="use" className="usage" aria-labelledby="usage-title"><div className="usage-intro"><h2 id="usage-title">Add an icon<br />{' '}to your project.</h2><p>Install the React package or download individual SVGs.<br />{' '}No separate stylesheet or animation library required.</p><a href={packageUrl} className="button primary">View on npm<Glyph name="down" size={16} /></a><small>v{packageVersion} · MIT license · React package</small></div><div className="usage-code"><div className="usage-code-title"><span><Glyph name="code" size={17} />React example</span></div><div className="install-line"><code>{installCommand}</code><CopyButton compact value={installCommand} label="Copy install command" onError={() => setMessage('Clipboard unavailable. Select the install command to copy it.')} /></div><pre tabIndex={0} aria-label="React usage example"><code><span className="syntax-muted">{'// Add to any React 18+ project\n'}</span><span className="syntax-purple">import</span>{' { DownloadIcon } '}<span className="syntax-purple">from</span>{'\n  '}<span className="syntax-green">{'\'@unlocalhosted/dither-icons\''}</span>{';\n\n'}{'<'}<span className="syntax-purple">button</span>{' className='}<span className="syntax-green">{'"di-trigger"'}</span>{'\n        aria-label='}<span className="syntax-green">{'"Download file"'}</span>{'>\n  <'}<span className="syntax-purple">DownloadIcon</span>{' size={48} />\n</button>'}</code></pre><div className="usage-code-foot"><span className="availability-dot" />Supports keyboard focus and reduced motion.</div></div></section>
