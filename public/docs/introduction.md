...
Install the local package, import a named component, and add it to a real button. The icon inherits your text color and needs no separate stylesheet.
...
> The package is currently a local download, not published to the npm registry. Named exports share the complete geometry catalog; per-icon bundle splitting is not implemented.
