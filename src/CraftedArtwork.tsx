...
export function CraftedArtwork({name,draw,texture}:Props){
 ...
 if(['reading-focus','start-at-text','listen','read-aloud','drag-handle','skip-block','collapse-rail','headphones'].includes(name))return <ReaderArtwork name={name} draw={draw} texture={texture}/>;
