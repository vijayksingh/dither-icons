...
4. Tag that exact commit with the version chosen above: `git tag -a vX.Y.Z -m 'Release vX.Y.Z'`, then `git push origin vX.Y.Z`. Replace `X.Y.Z` with that version. Never reuse or force-move release tags.
...
- **Cloudflare CI:** repository Actions secret `CLOUDFLARE_API_TOKEN` holds the dedicated token named `dither-icons GitHub Actions Pages`. Its only permission is **Account → Cloudflare Pages → Edit**, scoped to account `e3ae6e5dd1aac7f5d4c1d4aa45c00f83`. It has no client IP filter because GitHub-hosted runners use changing addresses. Only the deploy step receives this secret. Rotate it through Cloudflare and update the encrypted Actions secret; never print it or save it in this repo.
- **Cloudflare manual recovery:** the separate infra credential remains in `/opt/infra/secrets/cloudflare.env` on `vm-agent`, restricted to the infra machine's IP. Use the existing infra wrapper there. Do not reuse this credential in GitHub Actions or relax its IP restriction; Cloudflare rejects other runner addresses with error `9109`. Never ask the owner to paste either token into chat.
...
Use `gh run view <run-id> --log-failed` to identify the failed job. Fix missing external configuration and use `gh run rerun <run-id> --failed` when the tag and artifacts remain correct. npm publication is immutable: if publish succeeded but deployment failed, rerunning failed jobs resumes deployment from the same artifact without rerunning publication. If the publish job itself is retried, it verifies the existing package's integrity before accepting it.
