...
Outline correction: page border and both sound arcs now have transparent cores and 0.5-unit boundary rails inside their existing 1.5-unit footprint. Text is single-line at 0.9 units; underline uses 0.75. Wave masks remain inside their actors, preserving separation (MOT-01/05/07/12). Pixel tests sample the curved wave's transparent spine and visible boundary and distinguish materials at 16/24/48px. Timing is unchanged; new browser visual approval is not claimed.
...
## User Context
Keep the native Listen label and actual pressed/disabled state. Voice settings and playback errors remain application feedback. Solid uses inked bands, Outline uses hollow contour boundaries, and dither adds contour-bound grain. Compact recognition and cross-browser behavior remain unreviewed for this revision.
